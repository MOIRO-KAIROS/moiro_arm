...
MTCTaskNode::MTCTaskNode(const rclcpp::NodeOptions& options)
  : node_{ std::make_shared<rclcpp::Node>("mycobot_mtc_node", options) }
{
}
...
#pragma GCC diagnostic pop
  ...
  //   {
  //     auto stage = std::make_unique<mtc::stages::ModifyPlanningScene>("forbid collision object->gripper");
  //     stage->allowCollisions("object", 
  //                             task.getRobotModel()
  //                             ->getJointModelGroup(hand_group_name)
  //                             ->getLinkModelNamesWithCollisionGeometry(), 
  ...
  //     stage->setMinMaxDistance(0.1, 0.3);
  //     stage->setIKFrame(hand_frame);
  //     stage->properties().set("marker_ns", "retreat_after_place");
  ...
  // {
  //   auto stage = std::make_unique<mtc::stages::MoveTo>("return hime", interpolation_planner);
  //   stage->properties().configureInitFrom(mtc::Stage::PARENT, { "group" });
  //   stage->setGoal("ready");
