...
public:
    MyCobotMoveGroup() : Node("mycobot_move_group_service_class"), move_group_(std::shared_ptr<rclcpp::Node>(this), PLANNING_GROUP) {
        ...
        timer_ = this->create_wall_timer(
            500ms,
            std::bind(&MyCobotMoveGroup::check_and_request, this)
        );
    }
...
private:
    ...
    void request_target() {
        ...
        auto future = srv_client->async_send_request(
            request, std::bind(&MyCobotMoveGroup::handle_response, this, std::placeholders::_1)
        );
    }
    ...
    const std::string PLANNING_GROUP = "mycobot_arm";
...
int main(int argc, char **argv) {
    ...
    auto turtlebot3_controller = std::make_shared<MyCobotMoveGroup>();
